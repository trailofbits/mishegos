...
void m_cohort_print_json(FILE* f, cohort_results* r){
    ...
    for(int i = 0; i < r->nworkers; i++){
        if(i !=0){
            ...
            fprintf(f, "{ \"status\": { \"value\": %u, \"name\": \"%s\" }, \"ndecoded\": %u, \"workerno\": %u, \"worker_so\": \"%s\",\"len\": %ld, \"result\": \"%s\" }", 
            r->outputs[i].status, 
            status2str(r->outputs[i].status),
            r->outputs[i].ndecoded, 
            r->outputs[i].workerno, 
            r->outputs[i].workerso.string, 
            r->outputs[i].result.len,
...
void m_fread(void* ref, size_t size, size_t times, FILE* file){
    ...
    if(rd == 0 && size*times !=0){ //in cases of bad decompilation we want to read 0 length result string
...
/* 
    There are a few subtilties that this functions cathces
    ...
    we implicitly calloc 1 extra byte to guartentee 
...
void read_string(FILE* file, m_string* s, int len_size){
    ...
    char* input = calloc(1, sizeof(char) * string_length + 1); // calloc instead of malloc because we want to zero out the memory. 
    m_fread(input, sizeof(char), string_length, file);// this is because we tend to reuse the same memory alot (we optimize this call out probably)
